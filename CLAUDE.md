...
- 디자인 전략·비주얼 스펙: `apps/web/PRODUCT.md`(레지스터 brand·플랫폼 web·포지셔닝·안티레퍼런스·디자인 원칙)와 `apps/web/DESIGN.md`(북스타 "정서의 식물원", 토큰·타이포·컴포넌트 규칙). UI 작업 전 두 파일을 먼저 읽을 것. 핵심: 민트 액센트 ≤10%, Black Han Sans 는 히어로/섹션 헤딩 전용, 정지 상태 그림자 금지, WCAG 2.1 AA(본문 대비 4.5:1).
